...
export default {
  ...
  methods: {
    async accesptRequest(item) {
      ...
      // }
      await this.$store.dispatch("order/updateOrderUnderImplementing", {
        id: item._id,
      });
      this.$router.go();
    },
    ...
    async newOrder() {
      this.type = 'newOrder'
      await this.$store.dispatch("order/getOrderUnderImplementing", {type: ''})
    },
    async reNewOrder() {
      this.type = 'reNewOrder'
      await this.$store.dispatch("order/getOrderUnderImplementing", {type: 'تجديد الترخيص'})
    }

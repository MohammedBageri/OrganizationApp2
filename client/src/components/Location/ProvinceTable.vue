  ...
  >
    <template v-slot:top>
      <v-toolbar flat class="rounded-br-xl rounded-tl-xl">
          ...
          <v-row>
            ...
            <v-col cols="6">
              <v-card
                ...
                width="80"
                elevation="2"
                class="mr-6 mb-10 rounded-l text-center hidden-md-and-down"
              >
                <v-icon large class="mt-5" color="#14425a">mdi-map</v-icon>
              </v-card>
            </v-col>
          </v-row>
        </v-toolbar-title>
        ...
        <v-spacer></v-spacer>
        <v-dialog
          v-model="$store.state.stateDialog"
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      v-model="$store.state.state.name"
                      label="اسم المحافظة"
                      outlined
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="text-h5"
              >هل انت متأكد من حذف المحافظة؟</v-card-title
            >
            <v-card-actions>
              <v-spacer></v-spacer>
      ...
      <v-snackbar v-model="snackbarEdit" shaped top color="green">
        ...
        تم تعديل معلومات المحافظة بنجاح
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "إضافة محافظة" : "تعديل محافظة";
    },
  },
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.$store.state.stateDialog = true;
    },
    ...
    async deleteItemConfirm() {
      ...
      let result = await this.$store.dispatch("DeleteState", { id });
      if (result) {
    ...
    close() {
      this.$store.state.stateDialog = false;
      this.$nextTick(() => {
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        let id = this.$store.state.state._id;
        await this.$store.dispatch("UbdateState", { name, id });
        Object.assign(
        ...
        let name = this.$store.state.state.name;
        await this.$store.dispatch("CreateState", { name });

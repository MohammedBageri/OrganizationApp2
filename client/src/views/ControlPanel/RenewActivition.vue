<template>
    <ControlPanelLayout>
      <v-container class="mt-16 ml-10" style="height: 100%">
...
import axios from 'axios';
  ...
  export default {
    ...
    async mounted () {
        try {
        const organization = await this.$store.dispatch('organization/getOrganizationByEmail')
    ...
    methods: {
      ...
      async renew(){
        const id = this.$store.state.organization.organization._id
        await axios.post(`/api/Orders/renew_license/${id}`)
        .then(res => {
